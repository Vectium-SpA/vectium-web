  ...
  return (
    <div className="max-w-4xl mx-auto">
      <motion.div
        ...
        className="mb-6"
      >
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Buscar</h1>
      ...
      <TabGroup selectedIndex={selectedTab} onChange={setSelectedTab}>
        <TabList className="flex space-x-1 bg-gray-100 dark:bg-gray-800 p-1 mb-6 rounded-xl">
          <Tab
            className={({ selected }) =>
              classNames(
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {['Paracetamol', 'Ibuprofeno', 'Amoxicilina', 'Omeprazol'].map((suggestion) => (
                <button
                  ...
                  className="px-3 py-1.5 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 rounded-full text-sm hover:bg-farmateca-primary/10 hover:text-farmateca-primary transition-colors"
                >
                  {suggestion}
                </button>
              ))}
      ...
      {noResults && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12">
          ...
          <p className="text-gray-500 dark:text-gray-400">No hay coincidencias para &quot;{query}&quot;. Intenta con otro término.</p>
        </motion.div>
      )}
          ...
          {showCompounds && (
            <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Compuestos</h2>
                <span className="text-sm text-gray-500 dark:text-gray-400">({totalCompoundResults})</span>
              </div>
          ...
          {showBrands && (
            <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: showCompounds ? 0.1 : 0 }}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Marcas</h2>
                <span className="text-sm text-gray-500 dark:text-gray-400">({totalBrandResults})</span>
              </div>
              <div className="space-y-2">
