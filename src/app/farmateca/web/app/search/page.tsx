...
import { SearchBar, CompoundCard, BrandCard } from '@/components/farmateca/clinical';
...
export default function SearchPage() {
  ...
  const showCompounds = filteredResults.compounds.length > 0;
  const showBrands = filteredResults.brands.length > 0;
      ...
      {!hasSearched && !query && (
        ...
        >
          ...
          <p className="text-gray-500 max-w-sm mx-auto">
            Escribe el nombre de un compuesto o marca para encontrar información
            farmacológica detallada.
          </p>
      ...
      {noResults && (
        ...
        >
          ...
          <p className="text-gray-500">
            No hay coincidencias para &quot;{query}&quot;. Intenta con otro término.
          </p>
        </motion.div>
      )}
        ...
        <div className="space-y-8">
            ...
            >
              <div className="flex items-center gap-2 mb-4">
                <div className="w-8 h-8 bg-farmateca-compound/10 rounded-lg flex items-center justify-center">
                  <svg
                    className="w-4 h-4 text-farmateca-compound"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                ...
                <span className="text-sm text-gray-500">
                  ({filteredResults.compounds.length})
                </span>
              </div>

              <div className="space-y-2">
                {filteredResults.compounds.map((compound) => (
          ...
          {showBrands && (
            <motion.section
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: showCompounds ? 0.1 : 0 }}
            >
              <div className="flex items-center gap-2 mb-4">
              ...
              <div className="space-y-2">
                {filteredResults.brands.slice(0, 50).map((brand) => (
                  <BrandCard key={brand.idMA} brand={brand} />
                ))}
                {filteredResults.brands.length > 50 && (
                  <p className="text-center text-sm text-gray-500 py-2">
                    Mostrando 50 de {filteredResults.brands.length} resultados.
