...
import { SearchBar, BrandCard } from '@/components/farmateca/clinical';
...
export default function BrandSearchPage() {
  ...
  // NUEVO: Estado para filtro por laboratorio (Premium)
  ...
  // Cargar laboratorios desde JSON local al montar (para tab Premium)
  useEffect(() => {
  ...
  useEffect(() => {
    if (selectedIndex !== 0) return; // Solo tab "COMERCIAL"
    ...
    const timeoutId = setTimeout(async () => {
      ...
      try {
        ...
        // Filtrar solo comerciales en cliente usando tipoM
  ...
  useEffect(() => {
    if (selectedIndex !== 1) return; // Solo tab "GENÉRICO"
    ...
    const timeoutId = setTimeout(async () => {
      ...
      try {
        ...
        // Filtrar solo genéricos en cliente usando tipoM
  ...
  const noResultsCommercial =
    hasSearchedCommercial && commercialBrands.length === 0 && !isLoadingCommercial;
  const noResultsGeneric =
    hasSearchedGeneric && genericBrands.length === 0 && !isLoadingGeneric;
      ...
      <TabGroup selectedIndex={selectedIndex} onChange={setSelectedIndex}>
        ...
        <TabPanels>
          ...
          <TabPanel>
            ...
            >
              <SearchBar
                onSearch={handleSearchCommercial}
                placeholder="Buscar marca comercial..."
            ...
            {!hasSearchedCommercial && !searchQueryCommercial && (
              ...
              >
                ...
                <p className="text-gray-500 max-w-sm mx-auto">
                  Escribe el nombre de una marca comercial.
                </p>
              </motion.div>
            )}
            ...
            {/* Resultados */}
            {!isLoadingCommercial && hasSearchedCommercial && commercialBrands.length > 0 && (
              <div className="space-y-2">
                ...
                {commercialBrands.slice(0, 50).map((brand) => (
          ...
          <TabPanel>
            ...
            {!isLoadingGeneric && hasSearchedGeneric && genericBrands.length > 0 && (
              <div className="space-y-2">
                <div className="flex items-center gap-2 mb-4">
                ...
                {genericBrands.slice(0, 50).map((brand) => (
                  <BrandCard key={brand.idMA} brand={brand} />
                ))}
              </div>
            )}
