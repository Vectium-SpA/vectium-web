...
import { FlaskConical, Info, Activity, Package, Ban, Cross } from 'lucide-react';
  ...
  if (error || !brand) {
    return (
      <div className="text-center py-12">
        ...
        <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">Marca no encontrada</h2>
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        ...
        <div className="flex items-start gap-4 relative">
          <div className="w-14 h-14 bg-farmateca-primary/10 rounded-xl flex items-center justify-center flex-shrink-0">
            <svg className="w-7 h-7 text-farmateca-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
            </svg>
          </div>
          <div className="flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">{brand.ma}</h1>
      ...
      <div className="space-y-3">
        ...
        {/* 1. Botón "Ver Principio Activo" — FREE */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <svg className="w-5 h-5 text-farmateca-compound group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {/* 2. Información General — FREE (Principio Activo, Familia, Laboratorio) */}
        <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-5">
          <h2 className="text-base font-semibold text-gray-900 dark:text-white mb-4">Información General</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">Principio Activo</p>
              <Link href={`/farmateca/web/app/compound/${brand.idPAM}`} className="text-farmateca-primary font-medium hover:underline">
                {brand.paM}
              </Link>
            </div>
            <div>
        ...
        {/* 4. Vía de Administración — PREMIUM */}
        {brand.viaM && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
            <PremiumSection
              title="Vía de Administración"
              icon={<Activity className="w-4 h-4 text-farmateca-primary" />}
              featureName="Vía de Administración"
            >
              <p className="text-gray-700 dark:text-gray-300">{brand.viaM}</p>
            </PremiumSection>
          </motion.section>
        )}

        {/* 5. Presentación — PREMIUM */}
        {brand.presentacionM && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
            <PremiumSection
              title="Presentación"
              icon={<Package className="w-4 h-4 text-farmateca-primary" />}
              featureName="Presentación"
            >
        ...
        {brand.contraindicacionesM && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
            ...
            >
              <div className="text-red-700 dark:text-red-300 whitespace-pre-line leading-relaxed">{brand.contraindicacionesM}</div>
            </PremiumSection>
          </motion.section>
        )}
