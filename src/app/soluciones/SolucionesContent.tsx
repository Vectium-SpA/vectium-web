...
export function SolucionesContent() {
  ...
  return (
    <section ref={ref} className="bg-site-surface py-24 lg:py-32">
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-4">
          {processSteps.map((item, index) => (
            <motion.div
              ...
              className="relative rounded-2xl border border-site-border bg-white/80 backdrop-blur-xl p-8 text-center shadow-sm transition-all hover:shadow-lg hover:-translate-y-1"
            >
              <span className="text-5xl font-bold text-site-muted">{item.step}</span>
              <div className="mx-auto mt-4 flex h-14 w-14 items-center justify-center rounded-xl bg-site-accent/10 text-site-accent">
                <item.icon size={28} />
              </div>
